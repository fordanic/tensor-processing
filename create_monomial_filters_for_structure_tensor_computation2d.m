function create_monomial_filters_for_structure_tensor_computation2d()
% CREATE_MONOMIAL_FILTERS_FOR_STRUCTURE_TENSOR_COMPUTATION2D Creates 2D monomial filters for structure tensor estimation
%
% create_monomial_filters_for_struncture_tensor_computation2d()
%
...
for k = 1 : length(scales)
    ...
    fprintf('Optimizing monomials for structure tensor computation in 2d\n');
    ...
    [Ys, Xs] = getgrid(wa(frequencySize,1));
    
    Xs = Xs/spacing(1);
    Ys = Ys/spacing(2);
    
    Rs = sqrt(Ys.*Ys + Xs.*Xs) + eps;
...
save monomialsForStructureTensor2D coarse intermediate fine
